...
namespace VirtualCompress
{
    ...
    /// </summary>
    public class VirtualCompress
    {
        private static E_CompressConfig _config;
        ...
        private void ProcessSoftList()
        {
            foreach (var item in fileList)
            {
                ...
                while (SleepCnt > 0 && GetFreeMemory(fi.Length) < 0)
                {
                    ...
                    if (SleepCnt <= 0)
                    {
                        Util.SaveLog(_config.ErrorLogSavePath,$"当前内存不足,剩余内存为:{GetFreeMemory()}MB(已排除保留内存{_config.ReserveMemory})");
                        GC.Collect();
                        throw new OutOfMemoryException();
                    }
                }
                if (_config.CRCLevel == E_Const.EnumCRCLevel.RealTime)
                {
        ...
        private float GetFreeMemory(long length = 0)
        {
            ...
            //保留1的内存空间
            return (sysInfo.MemoryAvailable - length - _config.ReserveMemory) / 1024 / 1204;
        }
        ...
        private E_LocalFileHeader FileHeaderInstance(E_SourceFile file)
        {
            ...
            l.FileNameLength = BitConverter.GetBytes((short)file.FileName.Length);
            //文件扩展区长度默认为0即可
            l.FileName = Encoding.Default.GetBytes(file.FileName);
            //数据区填充null
            l.ExtraField = null;
            return l;
        }
        ...
        private E_CentralDirFileHeader CentralDirInstance(E_SourceFile file)
        {
            ...
            c.FileNameLength = BitConverter.GetBytes((short)file.FileName.Length);
            //扩展域长度,文件注释长度,开始位置的磁盘编号,为0即可
            c.ExternalFileAttr = BitConverter.GetBytes(0x00000020);
            //本地文件头的相对位移具体地方再做计算
            c.FileName = Encoding.Default.GetBytes(file.FileName);
            //扩展域和文件注释内容为null即可
            return c;
        }
